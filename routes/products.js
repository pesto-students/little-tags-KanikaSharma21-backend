...
router.delete("/:productId", adminAuth, async (req, res) => {
  ...
  let product = await Product.findOne({ _id: req.params.productId, status: "active" });
  if (!product)
    return res
      .status(400)
      .send({ statusCode: 400, message: "Failure", data: { data: PRODUCT_CONSTANTS.PRODUCT_NOT_FOUND } });

  if (req.query.isProductInCart == true) {
    let productInCart = await User.find({ cart: { $elemMatch: { productId: req.params.productId } } });
    console.log(productInCart);
    if (productInCart.length > 0) {
      return res.send({ statusCode: 400, message: "Failure", data: { data: PRODUCT_CONSTANTS.PRODUCT_IN_CART } });
    } else {
      return res.send({ statusCode: 400, message: "Failure", data: { data: PRODUCT_CONSTANTS.NO_PRODUCT_IN_CART } });
    }
  }

  await Product.updateOne({ _id: req.params.productId }, { $set: { status: "inactive" } });
  return res.send({ statusCode: 200, message: "Success", data: { data: PRODUCT_CONSTANTS.PRODUCT_DELETED } });
});
